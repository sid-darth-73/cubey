  ...
  return (
    <div>
      <div className="bg-slate-800 p-4 rounded-lg mb-8">
        <h3 className="text-2xl font-mont text-center font-semibold mb-4">Add a new solve</h3>
        <div className="flex flex-col md:flex-row gap-6 items-start">
              ...
              {dropdownOpen && (
                <div className="absolute mt-12 z-20 bg-white dark:bg-slate-700 rounded-lg shadow w-36">
                  <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
                    {eventOptions.map((ev) => (
                      <li key={ev}>
                        <button onClick={() => { setType(ev); setDropdownOpen(false); }} className="w-full text-left block px-4 py-2 hover:bg-gray-100 dark:hover:bg-slate-600 dark:hover:text-white">
                          {ev}
                        </button>
                      </li>
                    ))}
          ...
          <div className="w-full md:w-auto p-3 flex flex-col items-center md:items-end bg-slate-900/80 rounded-lg shadow-lg flex-shrink-0">
            ...
            <p className="text-xs font-mont text-slate-400 mt-1">import csv: time,type,scramble</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {solves.length === 0 ? (
          ...
          solves.map((solve) => {
            ...
            return (
              <div key={solve._id} className="bg-slate-800 p-4 rounded-lg border border-slate-700 hover:bg-slate-700 transition">
