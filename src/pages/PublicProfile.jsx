...
export function PublicProfile(){
  ...
  if(!data) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-900 via-slate-800 to-gray-900 text-white">
        <div className="bg-slate-800 p-6 md:p-8 w-full max-w-xl rounded-2xl shadow-xl border border-slate-600 backdrop-blur-sm text-center">
          <h1 className="text-2xl text-slate-300"> Cooking up your PBs...</h1>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-6 items-center">
          ...
          <div className="w-full flex flex-col gap-4">
            <div className="flex items-start gap-3 bg-slate-700 bg-opacity-30 p-3 rounded-lg">
              <div className="text-blue-400 font-semibold min-w-[90px]">Singles</div>
              <ul className="text-slate-100 text-base font-medium list-disc pl-4">
                {data.pbSolves.length === 0 ? (
                  <li className="text-slate-400 list-none">No PB solves yet.</li>
                ) : (
                  data.pbSolves.map((solve, idx) => (
                    <li key={idx}>{solve.type}: {solve.timeInSeconds.toFixed(2)}s</li>
                  ))
                )}
              </ul>
            </div>
            ...
            <div className="flex items-start gap-3 bg-slate-700 bg-opacity-30 p-3 rounded-lg">
              <div className="text-green-400 font-semibold min-w-[90px]">Averages</div>
              <ul className="text-slate-100 text-base font-medium list-disc pl-4">
                {data.averages.length === 0 ? (
                  <li className="text-slate-400 list-none">No PB averages yet.</li>
                ) : (
                  data.averages.map((avg, idx) => (
                    <li key={idx}>{avg.type}: {avg.timeInSeconds.toFixed(2)}s</li>
                  ))
                )}
              </ul>
            </div>
